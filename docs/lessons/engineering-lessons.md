...
## L-013 普通浏览器自动化需区分可访问性调用与真实输入

- 日期：2026-09-15
- 现象：同一校园网和普通 Edge profile 下，可访问性 `InvokePattern` 调用 ScienceDirect 的 `View PDF` 会打开 `Internal Server Error`，而 UI Automation 定位可见控件后发送系统级鼠标点击可以正常进入目标 PII 的签名 PDF。PDF 查看器保存还会打开系统“另存为”，浏览器下载目录配置不能代表最终目录。
- 做法：UI Automation 只负责按名称、可见性、启用状态、地址栏和 AutomationId 定位与复核，导航、保存和确认使用系统级键鼠输入；每一步之后重新核对目标 PII 和页面状态。保存时显式填写受控临时绝对路径，等待大小稳定并校验 `%PDF-` 与 `%%EOF` 后再交给业务服务，禁止把短期签名 URL 持久化。
- 适用范围：需要复用用户日常登录态、又不能通过调试协议连接的 Windows Chromium 桌面自动化流程。

## L-014 桌面自动化能力属于可恢复的运行环境依赖

- 日期：2026-09-16
- 现象：同一套普通 Edge 自动化代码在可访问 Windows 桌面的进程中能够完成下载，但后端进程运行在受限会话时无法通过 UI Automation 连接刚打开的文章标签页。将该错误归入普通远端失败会让同一任务无法在修复运行环境后继续。
- 做法：把浏览器缺失、启动失败和交互式桌面不可访问统一归类为 `browser_unavailable`，条目与任务进入 `waiting_user`；恢复按钮采用不预设具体处理动作的“继续下载”。真正的控件、页面和保存流程错误仍保持终态失败，避免掩盖实现缺陷。
- 适用范围：依赖交互式桌面、可见浏览器或系统级输入的异步任务。

## L-015 外部稳定标识应在使用边界规范化

- 日期：2026-09-16
- 现象：Scopus 对同一期刊同时返回 17 位紧凑 PII 和带 ISSN、年份分隔符的 PII。全文注册表只接受纯字母数字，导致带符号 PII 的整个期号在创建任务前被误判为没有全文来源。
- 做法：原始采集记录保留来源值；在 ScienceDirect 引用解析边界分别严格匹配紧凑格式和已知带符号格式，只对匹配成功的值转换为紧凑 PII。不得直接删除任意非字母数字字符，否则畸形标识也可能被转换成看似合法的地址。
- 适用范围：同一 DOI、PII、ISSN 或其他外部标识存在多种等价表示，而原始数据仍需可追溯的采集与提供器流程。
