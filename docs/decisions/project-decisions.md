...
- 状态：已生效
- 日期：2026-08-30
- 内容：主项目 Python 环境为仓库根目录 `.venv`（依赖清单 `backend/requirements.txt`），前端依赖经 npm 安装；不使用 conda。
...
- 状态：已生效
- 日期：2026-09-16
- 内容：当前项目移除视频转笔记和视频任务列表的后端域、API、前端页面、路由、导航、模型场景、路径定义、测试及数据库表；专属代码、测试和设计文档按原相对路径集中到根目录 `video-notes-extracted/`，该目录不参与项目运行。
- 理由：当前项目不再需要视频处理能力，继续保留会增加系统工具依赖、维护面和产品导航复杂度；独立归档便于后续整体迁移。
- 影响：`/api/video-note-tasks` 与 `/video-notes*` 不再可用；现有论文、采集、论文分析、模型平台和共享后台任务执行器不受影响；Alembic 历史保留，迁移 `a8d4e6f1b203` 删除原有空视频任务表。
